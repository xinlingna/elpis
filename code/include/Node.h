...
typedef struct segment_sketch {
    ts_type *indicators; // 内存大小由num_indicators指定
    int num_indicators; // node_segment->num_indicators=4  series_segment->num_indicators=2 
} segment_sketch;
...
public:
    ...
    Node(Index *index, FILE *file, Node *parent, int mode); // tree+hnsw

    unsigned int node_size; // 节点包含的向量数
    ...
    unsigned long id;
    bool is_hnswed = false; // 判断当前叶子节点是否已经建立hnsw

    node_split_policy *split_policy;

    segment_sketch *node_segment_sketches;

    hercules_file_buffer *file_buffer; // buffer for leaf node  对应的 hercules_file_map
...
//NEW
    ...
    void addTS(Index * index, VectorWithIndex* vwi, bool add_in_child = false);
    ...
    VectorWithIndex *getTS(Index *pIndex) const;
    ...
    static Node *Read(Index *pIndex, FILE *pFile,int mode);
    char *getLeafGraphFullFileName(Index *index) const;

    char *setLeafFileName(Index *index) const;
