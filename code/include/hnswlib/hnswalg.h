...
namespace hnswlib {
    ...
    public:
        ...
        HierarchicalNSW(SpaceInterface<dist_t> *s,
            ...
            size_data_per_element_ = size_links_level0_ + data_size_ + sizeof(labeltype); // 每个元素在内存中占用的总字节数
            offsetData_ = size_links_level0_; // 数据（向量）的起始位置偏移
        ...
        ~HierarchicalNSW() {
            ...
            if (edgeWeightList_ != nullptr) {
                for (size_t i = 0; i < cur_element_count; i++) {
                    if (edgeWeightList_[i] != nullptr) {
                        free(edgeWeightList_[i]);  // 释放内层数组
                        edgeWeightList_[i] = nullptr;
                    }
                }
                free(edgeWeightList_);  // 释放外层指针数组
                edgeWeightList_ = nullptr;
            }
...
//                    if (candidate_id == 0) continue;
        ...
        tableint mutuallyConnectNewElement(const void *data_point, tableint cur_c,
            ...
            getNeighborsByHeuristic2(top_candidates, M_); // 从top_candidates中筛选出M_个邻居
...

            {
                ...
                setListCount(ll_cur,selectedNeighbors.size()); // 写入邻居数量
                tableint *data = (tableint *) (ll_cur + 1);    // 写入邻居ID到邻接表
        ...
        ///////////////////////////////////
        /* add weight to the bottom layer */
        void initialEdgeWeight(){

            this->edgeWeightList_=(Edge**)malloc(sizeof(Edge*)*this->cur_element_count);
            if (this->edgeWeightList_ == nullptr) {
                throw std::runtime_error("Memory allocation failed for edgeWeightList_");
                exit(-1);
            }
            for (size_t i = 0; i < this->cur_element_count; i++)
            {
                this->edgeWeightList_[i]=(Edge*)malloc(sizeof(Edge)*this->cur_element_count);
                    if (this->edgeWeightList_[i] == nullptr) {
                        throw std::runtime_error("Memory allocation failed for edgeWeightList_[i]");
                        exit(-1);
                    }
                memset(this->edgeWeightList_[i], 0, sizeof(Edge)*this->cur_element_count); //权重初始为0
            }   
        }
        ...
        void saveIndex(const std::string &location) {
            ...
            /* Write edgeWeightList_ to the file */
            for (size_t i = 0; i < cur_element_count; i++) {
                // Write each Edge array to the file
                output.write(reinterpret_cast<char*>(edgeWeightList_[i]), sizeof(Edge) * cur_element_count);
            }
        ...
        void loadIndex(const std::string &location, SpaceInterface<dist_t> *s, size_t ef, size_t max_elements_i=0) {
            ...
            input.seekg(sizeof(Edge)*cur_element_count*cur_element_count,input.cur);
            ...
            size_links_per_element_ = maxM_ * sizeof(tableint) + sizeof(linklistsizeint); /* maxM_ * neighborhoodID +  neighborhood Number*/
            ...
            /* load edgeWeightList_ */
            edgeWeightList_=(Edge**)calloc(cur_element_count,sizeof(Edge*)); 
            if(edgeWeightList_==nullptr){
                throw std::runtime_error("Not enough memory: loadIndex failed to allocate edgeWeightList_");
                exit(-1);
            }
            unsigned int edgeWeightSize=sizeof(Edge) * cur_element_count;
            for (size_t i = 0; i < cur_element_count; i++)
            {
                edgeWeightList_[i]=(Edge*)calloc(cur_element_count, sizeof(Edge)); //  (hnswalg.h:891)
                if(edgeWeightList_[i]==nullptr){
                    throw std::runtime_error("Not enough memory: loadIndex failed to allocate edgeWeightList_[i]");
                    exit(-1);
                }
                input.read(reinterpret_cast<char*>(edgeWeightList_[i]),edgeWeightSize);
            }
...
//        static const unsigned char REUSE_MARK = 0x10;
         ...
         * 'label' is the external identifier (logical ID) of the data, typically provided by the user.
         * @param label
         */
         ...
         * 'internalId' is the internal index used by the system to locate the data in memory.
         * @param internalId
         */
        ...
        /* 
         * 更新ID为internalId的特征向量为dataPoint
         * 以dateNeighborProbability概率，更新一阶邻居的连接
         */
        void updatePoint(const void *dataPoint, tableint internalId, float updateNeighborProbability) {
            ...
            if (entryPointCopy == internalId && cur_element_count == 1) // 如果只有一个节点，不需要更新其邻居关系
                return;

            int elemLevel = element_levels_[internalId];
            std::uniform_real_distribution<float> distribution(0.0, 1.0);
            for (int layer = 0; layer <= elemLevel; layer++) {
                std::unordered_set<tableint> sCand; // 包含internalId、internalId的一阶邻居、internalId的二阶邻居
                std::unordered_set<tableint> sNeigh; // 只包含internalId的一阶邻居
...
//                        continue;
        ...
        void repairConnectionsForUpdate(const void *dataPoint, tableint entryPointInternalId, tableint dataPointInternalId, int dataPointLevel, int maxLevel) {
            ...
            if (dataPointLevel < maxLevel) {
                ...
                for (int level = maxLevel; level > dataPointLevel; level--) {  // beamwidth=1进行贪心搜索
                    bool changed = true;
                    while (changed) {
                        changed = false;
                        unsigned int *data;
            ...
            for (int level = dataPointLevel; level >= 0; level--) {
                std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> topCandidates = searchBaseLayer(
                        currObj, dataPoint, level); // (dist,ID)
        ...
        /*  get linklist of  internalId  in level Level*/
        ...
        /* 
         * level: 用户指定的level，如果等于-1，函数自行指定
         * label: 'label' is the external identifier (logical ID) of the data, typically provided by the user.
         * 
        */
        tableint addPoint(const void *data_point, labeltype label, int level) {
            ...
            tableint cur_c = 0; 
            {
                ...
                std::unique_lock <std::mutex> templock_curr(cur_element_count_guard_); // 哈希锁分配策略
                auto search = label_lookup_.find(label);
                if (search != label_lookup_.end()) {   //(label, existingInternalId)
            ...
            if (level > 0) // 用户指定level
                curlevel = level;

            element_levels_[cur_c] = curlevel; // (id,level)
            ...
            // Initialisation of the data and label in　the level 0
            memcpy(getExternalLabeLp(cur_c), &label, sizeof(labeltype));
            memcpy(getDataByInternalId(cur_c), data_point, data_size_);


            // level>0，为其高层邻接表单独分配内存
            if (curlevel) { 
            ...
            if ((signed)currObj != -1) {
                ...
                for (int level = std::min(curlevel, maxlevelcopy); level >= 0; level--) {
                    ...
                    // 搜索level层 data_point的top_candidates，enterpoint=currObj
                    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates = searchBaseLayer(
                            currObj, data_point, level);
                    if (epDeleted) {
                        top_candidates.emplace(fstdistfunc_(data_point, getDataByInternalId(enterpoint_copy), dist_func_param_), enterpoint_copy);
                        if (top_candidates.size() > ef_construction_) // ef_construction_: 候选池的大小
                            top_candidates.pop();
                    }
