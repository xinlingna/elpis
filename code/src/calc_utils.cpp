...
/* 判断 to 是否在给定的 points 数组中，如果在，就返回 from，否则返回 to。*/
...
计算两个timeseries的欧式距离
*/
/* double euclideanDistance(ts_type * vec1, ts_type * vec2, int timeseries_dim) {
    double sum = 0.0;
    for (size_t i = 0; i < timeseries_dim; i++) {
        sum += pow(vec1[i] - vec2[i], 2);
    }
    return sqrt(sum);
} */

float euclideanDistance(const float* vec1, const float* vec2, int dim) {
    __m256 sum_vec = _mm256_setzero_ps();  // 累加向量
    int i = 0;
